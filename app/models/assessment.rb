    ...
    if assessment.evaluation_status_changed? && !assessment.evaluation_status_was.nil?
      ...
      when Assessment::STATUS_BEING_EVALUATED
        if assessment.evaluation_status != Assessment::STATUS_APPROVAL_NEEDED
          assessment.errors[:evaluation_status] <<
            "Can only change status to #{
              Assessment::STATUS_APPROVAL_NEEDED
            } when assessment has status #{Assessment::STATUS_BEING_EVALUATED}"
        end

        if assessment.statement.statement_type == Statement::TYPE_FACTUAL ||
           assessment.statement.statement_type == Statement::TYPE_NEWYEARS
          if !assessment.veracity || !assessment.short_explanation || !assessment.explanation_html
            assessment.errors[:evaluation_status] <<
              "To be able to change status to #{
                Assessment::STATUS_APPROVAL_NEEDED
              }, please fill veracity, short_explanation, and explanation"
          end
        end
        if assessment.statement.statement_type == Statement::TYPE_PROMISE
          if !assessment.promise_rating || !assessment.short_explanation ||
             !assessment.explanation_html
            assessment.errors[:evaluation_status] <<
              "To be able to change status to #{
                Assessment::STATUS_APPROVAL_NEEDED
              }, please fill promise rating, short_explanation, and explanation"
          end
        end
      when Assessment::STATUS_APPROVAL_NEEDED
        if assessment.evaluation_status != Assessment::STATUS_BEING_EVALUATED &&
           assessment.evaluation_status != Assessment::STATUS_PROOFREADING_NEEDED
          assessment.errors[:evaluation_status] <<
            "Can change status either to #{Assessment::STATUS_BEING_EVALUATED} or #{
              Assessment::STATUS_PROOFREADING_NEEDED
            } when assessment has status #{Assessment::STATUS_APPROVAL_NEEDED}"
        end
      when Assessment::STATUS_PROOFREADING_NEEDED
        if assessment.evaluation_status != Assessment::STATUS_BEING_EVALUATED &&
          ...
          assessment.errors[:evaluation_status] <<
            ...
            } when assessment has status #{Assessment::STATUS_PROOFREADING_NEEDED}"
        end
      when Assessment::STATUS_APPROVED
        if assessment.evaluation_status != Assessment::STATUS_BEING_EVALUATED
          assessment.errors[:evaluation_status] <<
            "Can only change status to #{
              Assessment::STATUS_BEING_EVALUATED
            } when assessment has status #{Assessment::STATUS_APPROVED}"
        end

        if assessment.statement.published
          assessment.errors[:evaluation_status] <<
            "Cannot change status of published statement, unpublish before changing it"
        end
      else
...
class Assessment < ApplicationRecord
  ...
  has_and_belongs_to_many :secondary_evaluators, class_name: "User"
  ...
  validates :veracity,
            ...
            unless:
              Proc.new { |a|
                a.assessment_methodology.rating_model ==
                  AssessmentMethodology::RATING_MODEL_VERACITY
              }
  validates :promise_rating,
            absence: true,
            unless:
              Proc.new { |a|
                a.assessment_methodology.rating_model ==
  ...
  def is_user_authorized_to_save(user)
    ...
    evaluator_allowed_attributes = %w[
      veracity_id
      promise_rating_id
      explanation_html
      ...
      (
        ...
        (
          evaluation_status_was == STATUS_APPROVAL_NEEDED &&
            ...
            changed_attributes.keys == %w[evaluation_status]
        )

    if evaluator_allowed_changes && permissions.include?("statements:edit-as-evaluator") &&
       user_id == user.id
      return true
    end

    texts_allowed_attributes = %w[explanation_html explanation_slatejson short_explanation]
    texts_allowed_changes =
      unapproved? && (changed_attributes.keys - texts_allowed_attributes).empty?
    proofreader_allowed_changes =
      texts_allowed_changes || (evaluation_status_was == STATUS_PROOFREADING_NEEDED)
  ...
  def is_user_authorized_to_view_evaluation(user)
    ...
    if user
      ...
      if permissions.include?("statements:view-evaluation-as-evaluator") && user.id == user_id
        return true
      end
    end
  ...
  def create_notifications(current_user)
      ...
      unless user_id.nil?
        begin
          ...
          notifications <<
            Notification.new(
              statement_text:
                "#{current_user.display_in_notification} tě vybral/a jako ověřovatele/ku",
              full_text:
                "#{current_user.display_in_notification} tě vybral/a jako ověřovatele/ku výroku #{
                  statement.display_in_notification
                }",
      ...
      unless user_id_was.nil?
        begin
          ...
          notifications <<
            Notification.new(
              statement_text:
                "#{current_user.display_in_notification} tě odebral/a z pozice ověřovatele/ky",
              full_text:
                "#{
                  current_user.display_in_notification
                } tě odebral/a z pozice ověřovatele/ky výroku #{statement.display_in_notification}",
              statement_id: statement.id,
    ...
    if evaluation_status_changed? && !evaluation_status_was.nil?
      ...
      evaluation_status_label =
        ...
        when STATUS_BEING_EVALUATED
          "ve zpracování"
        when STATUS_APPROVAL_NEEDED
          "ke kontrole"
        when STATUS_PROOFREADING_NEEDED
          "ke korektuře"
      ...
      if evaluation_status == STATUS_APPROVAL_NEEDED
        statement.source.experts.each do |expert|
          notifications <<
            Notification.new(
              statement_text:
                "#{current_user.display_in_notification} změnil/a stav na #{
                  evaluation_status_label
                }",
              full_text:
                "#{current_user.display_in_notification} změnil/a stav tebou editovaného výroku #{
                  statement.display_in_notification
                } na #{evaluation_status_label}",
              statement_id: statement.id,
              recipient: expert
            )
        end
      ...
      if user_id
        notifications <<
          Notification.new(
            statement_text:
              "#{current_user.display_in_notification} změnil/a stav na #{evaluation_status_label}",
            full_text:
              "#{current_user.display_in_notification} změnil/a stav tebou ověřovaného výroku #{
                statement.display_in_notification
              } na #{evaluation_status_label}",
            statement_id: statement.id,
        ...
        User.active.where(notify_on_approval: true).each do |user|
          notifications <<
            Notification.new(
              ...
              full_text:
                "#{current_user.display_in_notification} změnil/a stav výroku #{
                  statement.display_in_notification
                } na #{evaluation_status_label}",
              statement_id: statement.id,
              recipient: user
  ...
  def post_to_proofreading_slack
    if evaluation_status_previously_changed? && evaluation_status == STATUS_PROOFREADING_NEEDED
      proofreading_needed_count =
        statement.source.statements.select do |s|
          s.assessment.evaluation_status == STATUS_PROOFREADING_NEEDED
        end.size

      if proofreading_needed_count > 0 && proofreading_needed_count % 5 == 0
        source_url =
          "https://demagog.cz/admin/sources/#{
            statement.source.id
          }?filter=%7B%22field%22%3A%22assessment.evaluationStatus%22%2C%22value%22%3A%22proofreading_needed%22%7D"
        SlackNotifier::ProofreadingNotifier.post text:
                                                   "<!channel> Ahoj, máme tu v diskuzi *#{
                                                     statement.source.name
                                                   }* už #{
                                                     proofreading_needed_count
                                                   } výroků ke korektuře. Prosíme o projití. Díky!\n#{
                                                     source_url
                                                   }"
      end
    end
